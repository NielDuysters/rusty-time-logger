...
pub fn save(
    ...
    let csv_file_path = config::RUSTY_TIME_LOGGER_PATH
        .join("timelogs")
        .join(project_id.to_uppercase());
    ...
    writeln!(
        csv_file,
        "{},{},{},{}",
        row_id, date, task_description, time_string
    )?;
...
pub fn delete(project_id: &str, task_id: &str) -> Result<(), String> {
    ...
    let temp_file_path = config::RUSTY_TIME_LOGGER_PATH
        .join("timelogs")
        .join(format!(".{}.tmp", project_id.to_uppercase()));
    ...
    for line in reader.lines() {
        match line {
            Ok(content) => {
                let line_parts: std::vec::Vec<String> =
