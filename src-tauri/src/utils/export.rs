...
pub fn export_to_html(project: &project_service::Project) {
    ...
    html = html.replace("%total_time_spent%", &time::seconds_to_his(project.total_seconds_spent().unwrap()));
    ...
    let tbody_time_per_task = project
        ...
        .iter()
        .map(|(task, time)| {
            format!("<tr><td>{}</td><td>{}</td></tr>", task, time::seconds_to_his(*time))
        })
        .collect::<String>();
    ...
    let tbody_tasks_log = project
        ...
        .rev()
        .map(|task| {
            format!("<tr><td>{}</td><td>{}</td><td>{}</td></tr>", task.get(1).unwrap(), task.get(2).unwrap(), task.get(3).unwrap())
        })
        .collect::<String>();
    html = html.replace("%tbody_tasks_log%", &*tbody_tasks_log);

    if let Some(download_dir) = dirs::download_dir() {
        let export_file_path = download_dir.join(format!("rusty-time-logger-export-{}.html", project.id));
        ...
        .create(true)
        .truncate(true)
        .open(export_file_path).expect("Failed to create export file.");
